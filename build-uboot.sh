...
function usage() {
       echo "Usage: $0 <friendlycore-focal_4.14_armhf|friendlywrt_4.14_armhf>"
       echo "# example:"
       ...
       echo "# then"
       echo "    ./build-uboot.sh friendlycore-focal_4.14_armhf "
       echo "    ./mk-emmc-image.sh friendlycore-focal_4.14_armhf "
       echo "# also can do:"
...
case ${TARGET_OS} in
friendlycore-focal_4.14_armhf | friendlycore-focal_4.14_armhf | friendlywrt_4.14_armhf | eflasher)
        ;;
*)
...
if [ $? -eq 0 ]; then
    echo "updating ${TARGET_OS}/u-boot-sunxi-with-spl.bin ok."
else
    echo "failed."
    exit 1
fi
